...
// Ottieni staff assegnato PER PACCHETTO (SOLO CON COSTO > 0)
$staff_by_package = [];
foreach ($packages as $pkg) {
    $staff_result = mysqli_query($db, "SELECT qsa.*, s.first_name, s.last_name, sr.name as role_name
                                       ...
                                       WHERE qsa.quote_item_id = {$pkg['id']} AND qsa.cost > 0
...
// Ottieni TUTTI i servizi per pacchetto (solo con costo > 0)
$services_result = mysqli_query($db, "SELECT * FROM quote_service_costs WHERE quote_id = $quote_id AND cost > 0 ORDER BY service_name");
$all_services = [];
while ($row = mysqli_fetch_assoc($services_result)) {
    $all_services[] = $row;
}

// CALCOLA TOTALE GENERALE (Costo) di TUTTI i pacchetti per proporzione provvigione
$grand_total_all_packages_cost = 0;
...
foreach ($packages as $pkg) {
    ...
    // Aggiungi servizi di QUESTO PACCHETTO (solo costo)
    ...
    $grand_total_all_packages_cost += $pkg_total_cost;
}
...
<head>
    <meta charset="UTF-8">
    <title>Scheda Evento OKL - <?php echo e($quote['quote_number']); ?></title>
    <style>
...
foreach ($packages as $pkg_idx => $package): 
    ...
    // PROVVIGIONE: Proporzionale al COSTO di questo pacchetto (solo costo per OKL)
    $commission_percentage = $grand_total_all_packages_cost > 0 ? ($grand_total_cost / $grand_total_all_packages_cost) : 0;
    $commission_this_package = ($quote['commercial_commission'] ?? 0) * $commission_percentage;
?>

    <h1>Scheda Evento - OKL</h1>
    ...
    <table>
        <tr>
            <th style="width: 18%;">Stato Evento</th>
            <th style="width: 22%;">Gestione Personale</th>
            <th style="width: 25%;">Importo Fattura</th>
            <th style="width: 35%;">Venditore</th>
        </tr>
        <tr>
    ...
    <?php if (($quote['deposit_amount'] ?? 0) > 0 && $quote['deposit_type'] == 'cost'): ?>
    <table>
        <tr>
            <th style="width: 30%;">Acconto Versato (Questo Evento)</th>
            <th style="width: 70%;">Note</th>
        </tr>
        <tr>
            <td class="text-right bold"><?php echo formatPrice($deposit_this_package); ?></td>
            <td class="small">
                <?php echo date('d/m/Y', strtotime($quote['deposit_date'])); ?>
                <?php if ($total_pages > 1): ?>
                • <strong>Totale: <?php echo formatPrice($quote['deposit_amount']); ?></strong> (diviso in <?php echo $total_pages; ?> eventi)
                <?php endif; ?>
            </td>
        </tr>
    </table>
    ...
    <?php if (($quote['commercial_commission'] ?? 0) > 0 && $quote['commission_type'] == 'cost'): ?>
    <table>
        <tr>
            <th style="width: 30%;">Provvigione Commerciale (Questo Evento)</th>
            <th style="width: 70%;">Note</th>
        </tr>
        <tr>
            ...
            <td class="small">
                Sottratta da Costi
                <?php if ($total_pages > 1): ?>
                • <strong>Totale: <?php echo formatPrice($quote['commercial_commission']); ?></strong> (<?php echo number_format($commission_percentage * 100, 1); ?>% del totale)
                <?php endif; ?>
            </td>
        </tr>
    </table>
    ...
    <table>
        ...
        <tr class="totals" style="background: #d0d0d0;">
            <td colspan="2" class="text-right bold">TOTALE GENERALE COSTI</td>
